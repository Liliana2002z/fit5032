...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>We operate in: {{ operatingCountries.join(', ') }}</p>
      <p>Our #1 seller: {{ topSellers[0] }}</p>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{ highlight: author.id === highlightedAuthorId }"
          :style="{ fontWeight: author.id === highlightedAuthorId ? 'bold' : 'normal', cursor: 'pointer' }"
...
import rawAuthors from "../assets/json/authors.json"
import bookstores from "../assets/json/bookstores.json"

const authors = ref(rawAuthors)
...
const operatingCountries = computed(() => bookstores.countries)

// Activity 2: Get authors born after 1850
const modernAuthors = computed(() => {
  // TODO: CODE TO FILTER ARRAY OF AUTHORS HERE
  return authors.value.filter((author) => author.birthYear > 1850)
})

// Activity 3: Get all famous works
const allFamousWorks = computed(() => {
  // TODO: CODE TO GET ALL FAMOUS WORKS HERE
  return authors.value.flatMap((author) => author.famousWorks.map((work) => work.title))
})
...
const austen = computed(() => {
  ...
  return authors.value.find((author) => author.name === "Jane Austen")
})
</script>
